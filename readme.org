...
#+end_src
   ...
   scaling than if we used the brute force method with scaling. In total this
   speeds up the large image search by making it a matter of a minute or two,
   instead of dozens or possibly an hour (I wasn't patient enough to let the
   brute force method finish).

All images can be found in [[file:./data/processed]]. Below are the vectors used:
